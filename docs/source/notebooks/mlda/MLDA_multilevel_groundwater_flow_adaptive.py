...
def my_loglik(my_model, theta, datapoints, data, sigma, k, n):
    """
    ...
    return - ((0.5 / sigma ** 2) * np.sum((output - data) ** 2)) + np.sum(k) + np.sum(np.reshape(n, 4))
...
class LogLike(tt.Op):
    ...
    itypes = [tt.dvector, tt.dvector, tt.dmatrix]  # expects a vector of parameter values and a scalar input when called
    ...
    def perform(self, node, inputs, outputs):
        ...
        logl = self.likelihood(self.my_model, theta, self.x, self.data, self.sigma, m, s)
...
for j in range(len(my_models) - 1):
    with pm.Model() as model:
        mu_B = pm.Data('mu_B', np.zeros(nparam))
        Sigma_B = pm.Data('Sigma_B', np.identity(nparam))
        ...
        #temp = logl[j]
        #k0 = tt.as_tensor_variable(np.float64(0.))
        pm.DensityDist('likelihood', lambda v, m0=mu_B, s0=Sigma_B: logl[j](v, m0, s0), observed={'v': theta})
...
with pm.Model():
    ...
    # use a DensityDist (use a lamdba function to "call" the Op)
    #k0 = tt.as_tensor_variable(np.float64(0.))
    pm.DensityDist('likelihood', lambda v, m0=mu_B, s0=Sigma_B: logl[-1](v, m0, s0), observed={'v': theta})
